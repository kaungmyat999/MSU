...
const form_link = 'http://localhost:3000/review-event'
fetch('http://localhost:3000/events').then( d => d.json()).then(events => {
    table_datas =[]
    for(let i of events){
        table_datas.push([i.name,i.leadby,i.location,i.date])
    }
    return table_datas;
}).then(data => {
    new gridjs.Grid({
        columns: ["Name", "Leadby", "Location","Date"],
        pagination:{
            limit:6,
        },
        serach : true,
